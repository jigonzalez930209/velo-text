...
import { PLAYGROUND_LS } from "./seed.ts";

export function wirePanels(root: HTMLElement, editor: Editor, els: {
  status: HTMLElement;
  ...
  function persist(doc: PortableDocument): void {
    persistChain = persistChain.then(async () => {
      try { localStorage.setItem(PLAYGROUND_LS, JSON.stringify(doc)); } catch { /* quota */ }
      try {
  ...
  function refresh(doc: PortableDocument): void {
    ...
    refreshOutline();
    if (root.classList.contains("pg-preview-on") || root.classList.contains("pg-split")) refreshPreview();
    persist(doc);
  }

  root.querySelector("#view-editor")?.addEventListener("click", () => {
    root.classList.remove("pg-preview-on", "pg-split");
  });
  root.querySelector("#view-preview")?.addEventListener("click", () => {
    root.classList.add("pg-preview-on");
    root.classList.remove("pg-split");
    refreshPreview();
  });
  root.querySelector("#view-split")?.addEventListener("click", () => {
    root.classList.add("pg-split");
    root.classList.remove("pg-preview-on");
    refreshPreview();
  });
