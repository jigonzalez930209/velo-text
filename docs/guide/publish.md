...
Current version: **`1.0.3`** (npm dist-tag `beta`).
...
```bash
pnpm version:set 1.0.3
# edit CHANGELOG.md if needed
git add -A && git commit -m "release: 1.0.3"
git tag v1.0.3
...
2. Publishes to [npmjs.com/package/velo-text](https://www.npmjs.com/package/velo-text) with tag `beta` (any `package.json` version that contains `-` uses `beta`; a clean `1.0.3` uses `latest`).
...
- Next beta: bump to `1.0.3`, commit, tag `v1.0.3`, push the tag.
- Stable 1.0: set version `1.0.3` (no `-beta`), tag `v1.0.3` → CI publishes `latest`.
