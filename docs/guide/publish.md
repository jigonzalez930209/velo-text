...
Current version: **`1.0.1`** (npm dist-tag `beta`).
...
2. Publishes to [npmjs.com/package/velo-text](https://www.npmjs.com/package/velo-text) with tag `beta` (any `package.json` version that contains `-` uses `beta`; a clean `1.0.1` uses `latest`).
...
- Next beta: bump to `1.0.1`, commit, tag `v1.0.1`, push the tag.
- Stable 1.0: set version `1.0.1` (no `-beta`), tag `v1.0.1` → CI publishes `latest`.
