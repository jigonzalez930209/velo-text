...
function walkInlines(nodes: InlineNode[] | undefined, path: string, out: PdfDiag[], marksOnce: { n: boolean }): void {
  for (const n of nodes ?? []) {
    ...
    else if (n.type === "text" && (n as { marks?: object }).marks && !marksOnce.n) {
      marksOnce.n = true;
      out.push({ code: "pdf-marks-ignored", message: "Bold/italic/color marks are not applied in PDF (Helvetica only)", severity: "warning" });
    } else if (n.type === "link") walkInlines((n as { children?: InlineNode[] }).children, `${path}/${n.id}`, out, marksOnce);
  }
}

function walkBlocks(blocks: BlockNode[] | undefined, path: string, out: PdfDiag[], ctx: "root" | "cell" | "column", marksOnce: { n: boolean }): void {
  for (const b of blocks ?? []) {
    const p = `${path}/${b.id}`;
    if (b.type === "paragraph" || b.type === "heading" || b.type === "quote") {
      walkInlines((b as { children?: InlineNode[] }).children, p, out, marksOnce);
    } else if (b.type === "list") {
      for (const it of (b as { items: Array<{ id: string; content: InlineNode[]; nested?: BlockNode }> }).items) {
        walkInlines(it.content, `${p}/${it.id}`, out, marksOnce);
      ...
      for (const row of (b as { rows: Array<{ id: string; cells: Array<{ id: string; blocks: BlockNode[] }> }> }).rows) {
        for (const cell of row.cells) {
          ...
          walkBlocks(cell.blocks, `${p}/${cell.id}`, out, "cell", marksOnce);
        }
      }
      ...
      for (const col of (b as { columns: Array<{ id: string; blocks: BlockNode[] }> }).columns) {
        ...
        walkBlocks(col.blocks, `${p}/${col.id}`, out, "column", marksOnce);
      }
...
export function collectPdfDiagnostics(
  ...
  walkBlocks(doc.root.children, "root", out, "root", marksOnce);
