...
test("ux: image alt box hides on outside click", () => {
  ...
  assert(el.parentElement.querySelector(".pde-img-meta"), "alt/caption shown");
  el.ownerDocument.body.dispatchEvent(new el.ownerDocument.defaultView.MouseEvent("pointerdown", { bubbles: true }));
  assert(!el.parentElement.querySelector(".pde-img-meta"), "alt/caption hidden");
  editor.destroy();
  teardown();
});
...
test("ux: drag image into table cell and column", () => {
  ...
  dropAt(el.querySelector("td"), img.id);
  const afterCell = editor.getDocument();
  const table = afterCell.root.children.find((b) => b.type === "table");
  assert(table.rows[0].cells[0].blocks.some((b) => b.id === img.id), "image in cell");
