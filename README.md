...
**1.0.0-beta.2** — public beta. APIs and export layout can still change. Install with the `beta` tag, not as if it were a stable 1.0.
...
# exact prerelease
pnpm add velo-text@1.0.0-beta.2
```
...
Do **not** put an npm token in the repo. Create GitHub secret **`NPM_TOKEN`**, then push tag `v1.0.0-beta.2`. CI publishes to npm (`beta`) and opens a GitHub Release. Details: [docs/guide/publish.md](docs/guide/publish.md).
...
pnpm version:set 1.0.0-beta.2   # bump version (then tag v1.0.0-beta.2 and push)
```

Publish is **GitHub Actions only** (push tag `v1.0.0-beta.2` after setting secret `NPM_TOKEN`).
